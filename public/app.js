...
async function efetivoPage(){
  try{
    ...
    const grupos=["Comando","Sub-Comando","Supervisor","Piloto Oficial","Probatório","Outros"];
    ...
    rows.forEach(r=>{
      let grupo=r.cargo || r.unidade || '';
      if (String(grupo).toLowerCase()==='piloto probatório') grupo='Probatório';
      if (String(grupo).toLowerCase()==='piloto') grupo='Piloto Oficial';
...
function efetivoGrupo(grupo, rows){
  return `<div class="section efetivo-section"><div class="group-title"><h3>${escapeHtml(grupo)}</h3><span>${rows.length} integrante(s)</span></div>
    <div class="table-wrap"><table class="table efetivo-table"><thead><tr><th>ID</th><th>NOME</th><th>CARGO</th><th>TELEFONE</th><th>STATUS</th><th>CONTA</th></tr></thead><tbody>
    ${rows.map(r=>`<tr>
      <td><b>${escapeHtml(r.matricula || '—')}</b></td>
      <td><strong>${escapeHtml(r.nome)}</strong></td>
      <td>${escapeHtml(r.cargo || r.patente || '—')}</td>
